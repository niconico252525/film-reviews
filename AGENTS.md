...
normal GET fallback. Production deployment targets Render with Gunicorn,
PostgreSQL, and WhiteNoise.
...
* Python >= 3.9 (Render uses Python 3.12 from `.python-version`)
...
| Deployment check | `DJANGO_DEBUG=false SECRET_KEY=<secure-value> DJANGO_ALLOWED_HOSTS=localhost python manage.py check --deploy` |
| Collect static files | `python manage.py collectstatic --no-input` |
| Production server | `gunicorn app.wsgi:application` |
...
* `requirements.txt` — pip-compatible dependency list kept in sync with
  `pyproject.toml`.
* `requirements-production.txt` — runtime-only subset used by `build.sh`.
* `uv.lock` — reproducible dependency resolution generated by uv.
* `render.yaml` — Render web service and PostgreSQL Blueprint.
* `build.sh` — production dependency, static-file, check, and migration build.
