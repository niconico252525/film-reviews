...
Live application: <https://film-reviews-pfxo.onrender.com/>
...
The Blueprint generates `SECRET_KEY_SEED`; Django derives a 128-character
secret from that 256-bit random value; `DATABASE_URL` comes from the private
database; debug output is disabled; and each new `main` commit is redeployed.
No secret value belongs in Git or `render.yaml`.
...
| --- | --- | --- |
| `SECRET_KEY_SEED` | Render production | Random seed generated by the Blueprint and expanded into Django's secret key |
| `SECRET_KEY` | Other production hosts | Direct cryptographic Django secret when no generated seed is available |
...
```bash
curl --fail --show-error https://film-reviews-pfxo.onrender.com/
curl --fail --show-error --head https://film-reviews-pfxo.onrender.com/static/reviews/styles.css
```
...
The current free PostgreSQL instance expires on 21 August 2026 unless it is
upgraded. Upgrade the database before that date if the demonstration data must
remain available.
